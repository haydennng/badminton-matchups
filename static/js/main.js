...
async function loadMonthlyEarnings() {
    try {
        ...
        if (header) {
            header.textContent = `${monthName} Earnings`;
        }
        
        // Get earnings data
        const earnings = await api('/api/earnings');
        ...
        if (earnings.length === 0) {
            container.innerHTML = '<div style="color: var(--text-muted); font-size: 0.875rem; padding: 1rem;">No earnings data yet</div>';
            return;
        }
        
        // Build earnings display similar to matchups page
        ...
        const container = qs('#monthlyEarningsList');
        if (container) {
            container.innerHTML = '<div style="color: var(--text-muted); font-size: 0.875rem; padding: 1rem;">Unable to load earnings</div>';
        }
    }
}
...
// Load quick recap of previous session
async function loadQuickRecap(sessions) {
    const recapContent = qs('#quickRecapContent');
    
    try {
        const prevSession = getMostRecentNonTodaySession(sessions);
        
        if (!prevSession) {
            recapContent.innerHTML = '<div class="table-empty" style="padding: 1.5rem;">No previous session found</div>';
            return;
        }
        
        // Fetch session details
        const sessionDetail = await api(`/api/sessions/${prevSession.session_id}`);
        const earningsData = await api(`/api/sessions/${prevSession.session_id}/earnings`);
        
        const matches = sessionDetail.matches || [];
        const winStats = computePlayerWinStats(matches);
        
        // Build player performance data
        const playerPerf = [];
        (earningsData.players || []).forEach(playerEarnings => {
            const playerName = playerEarnings.player;
            const netEarnings = playerEarnings.net_earnings || 0;
            const playerWins = winStats[playerName] || { wins: 0, total: 0 };
            const winRate = playerWins.total > 0 ? playerWins.wins / playerWins.total : 0;
            
            const category = categorizePlayerPerformance(netEarnings, winRate);
            playerPerf.push({ name: playerName, category, netEarnings, winRate });
        });
        
        // Sort by category: green, orange, red
        const categoryOrder = { green: 0, orange: 1, red: 2 };
        playerPerf.sort((a, b) => categoryOrder[a.category] - categoryOrder[b.category]);
        
        // Render recap
        const dateStr = formatDate(prevSession.date);
        const pillsHtml = playerPerf.map(p => 
            `<span class="player-pill player-pill--${p.category}">${escapeHtml(p.name)}</span>`
        ).join('');
        
        recapContent.innerHTML = `
            <div style="padding: 1rem;">
                <div style="margin-bottom: 1rem; color: var(--text-muted); font-size: 0.95rem;">
                    <strong style="color: var(--text);">${dateStr}</strong> • ${matches.length} game${matches.length !== 1 ? 's' : ''}
                </div>
        ...
        recapContent.innerHTML = '<div style="padding: 1.5rem; color: var(--text-muted);">Unable to load recap</div>';
    }
}
...
async function loadPlayers() {
    try {
        ...
        if (players.length === 0) {
            tbody.innerHTML = '<tr><td colspan="2" class="table-empty">No players added yet</td></tr>';
            return;
        }
...
async function loadSessions() {
    try {
        ...
        const rowsHtml = await Promise.all(sessions.map(async session => {
            ...
            try {
                const sessionDetail = await api(`/api/sessions/${session.session_id}`);
                const matches = sessionDetail.matches || [];
                const winStats = computePlayerWinStats(matches);
                
                if (isMobile) {
                    // Mobile: Just show player count
                    const playerCount = Object.keys(winStats).length;
                    playersHtml = playerCount > 0 ? `${playerCount}` : 'No players';
                } else {
                    // Desktop: Classify players as winners (green) or losers (red)
                    const playerCategories = [];
                    Object.keys(winStats).forEach(playerName => {
                        const stats = winStats[playerName];
                        const netWins = stats.wins - (stats.total - stats.wins);
                        const category = netWins > 0 ? 'green' : 'red';
                        playerCategories.push({ name: playerName, category });
                    });
                    
                    // Sort: green first, then red
                    playerCategories.sort((a, b) => {
                        if (a.category === b.category) return a.name.localeCompare(b.name);
                        return a.category === 'green' ? -1 : 1;
                    });
                    
                    // Build pills HTML
                    const pillsHtml = playerCategories.map(p => 
                        `<span class="player-pill player-pill--${p.category}">${escapeHtml(p.name)}</span>`
                    ).join('');
            ...
            return `
                <tr data-session-id="${session.session_id}" class="session-row" onclick="toggleSession('${session.session_id}')" style="cursor: pointer;">
                    <td>${formatDateDesign(session.date)}</td>
                    <td>${playersHtml}</td>
                    <td style="text-align: center;">${session.match_count}</td>
...
async function loadSessionMatches(sessionId) {
    try {
        ...
        if (sessionData.matches.length === 0) {
            ...
            tbody.innerHTML = sessionData.matches.map(match => {
...
function renderAllCharts(raw) {
    const slices = computeStatsSlices(raw);
    
    const colorAccent = cssVar('--dg-500', '#40916C');
    const colorDanger = cssVar('--danger', '#E76F51');
    const colorSuccess = cssVar('--ok', '#2A9D8F');
    
    // Player Win Rates (Top)
    {
        const arr = slices.wr.top;
        if (arr.length > 0) {
            createHBarChart('chartWinrateTop', {
                chartKey: 'winTop',
                labels: arr.map(x => x.name),
                values: arr.map(x => x.winRate),
                color: colorAccent,
                xTickFormatter: (v) => fmtPercent(v),
                tooltipLabel: (i) => {
                    const x = arr[i];
                    return `${x.name}: ${fmtPercent(x.winRate)} • ${x.wins}-${x.losses} • ${x.games} games`;
                }
            });
        }
    }
    
    // Player Win Rates (Bottom)
    {
        const arr = slices.wr.bottom;
        if (arr.length > 0) {
            createHBarChart('chartWinrateBottom', {
                chartKey: 'winBottom',
                labels: arr.map(x => x.name),
                values: arr.map(x => x.winRate),
                color: colorDanger,
    ...
    // Player Money Performance (Top)
    {
        const arr = slices.money.top;
        if (arr.length > 0) {
            createHBarChart('chartMoneyTop', {
                chartKey: 'moneyTop',
                labels: arr.map(x => x.name),
                values: arr.map(x => x.net),
                color: colorSuccess,
                xTickFormatter: (v) => fmtCurrency(v),
                tooltipLabel: (i) => {
                    const x = arr[i];
                    return `${x.name}: ${fmtCurrency(x.net)} • ${x.wins}-${x.losses} (${fmtPercent(x.winRate)})`;
                }
            });
        }
    }
    
    // Player Money Performance (Bottom)
    {
        const arr = slices.money.bottom;
        if (arr.length > 0) {
            createHBarChart('chartMoneyBottom', {
                chartKey: 'moneyBottom',
                labels: arr.map(x => x.name),
                values: arr.map(x => x.net),
                color: colorDanger,
                xTickFormatter: (v) => fmtCurrency(v),
    ...
    // Partnership Performance (Top)
    {
        const arr = slices.pairs.top;
        if (arr.length > 0) {
            createHBarChart('chartPartnerTop', {
                chartKey: 'pairTop',
                ...
                tooltipLabel: (i) => {
                    const x = arr[i];
                    return `${x.label}: ${fmtPercent(x.winRate)} • ${x.wins}-${x.losses} (${x.games} games) • Together: ${fmtCurrency(x.earnings)}`;
                }
            });
        }
    }
    
    // Partnership Performance (Bottom)
    {
        const arr = slices.pairs.bottom;
        if (arr.length > 0) {
            createHBarChart('chartPartnerBottom', {
                chartKey: 'pairBottom',
                labels: arr.map(x => x.label),
                values: arr.map(x => x.winRate),
                color: colorDanger,
